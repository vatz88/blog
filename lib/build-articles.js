...
function buildArticlePages(dir) {
  ...
  dir.forEach(function(el) {
    if (fs.existsSync(path.resolve(__dirname, '..', el, 'index.json'))) {
      let indexjson = fs.readFileSync(path.resolve(__dirname, '..', el, 'index.json'));
      indexjson = JSON.parse(indexjson.toString());
      const readmetext = fs.readFileSync(path.resolve(__dirname, '..', el, 'README.md'));
      const readmehtml = converter.makeHtml(readmetext.toString());
      let html = htmlgenerator(indexjson);
      html = html.toString().split('<!--split-->');
      html = html[0] + readmehtml.toString() + html[1];

      fs.writeFile(path.resolve(__dirname, '..', el, 'index.html'), html, function(err) {
        if (err) {
          console.log(err);
        } else {
          count++;
...
function buildHomePage() {
  ...
  fs.writeFile(path.resolve(__dirname, '..', 'README.md'), readmetext, function(err) {
    if (err) {
      console.log(err);
    } else {
  ...
  fs.writeFile(path.resolve(__dirname, '..', 'index.json'), JSON.stringify(indexjson), function(
    err
  ) {
    if (err) {
      console.log(err);
  ...
  fs.writeFile(path.resolve(__dirname, '..', 'index.html'), html, function(err) {
    if (err) {
      console.log(err);
    } else {
